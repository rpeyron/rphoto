...
bool wxConfigDialog_EntryCheck::loadFromConfig()
{
    bool state;
    state = defaultState;
    getConfig().Read(groupId+"/"+entryId, &state);
...
{
    getConfig().Write(groupId+"/"+entryId, m_pCheck->GetValue());
    return TRUE;
}
...
{
    getConfig().Write(groupId+"/"+entryId, m_pRadio->GetSelection());
    return TRUE;
}
...
bool wxConfigDialog_EntryTextEdit::loadFromConfig() 
{
    wxString val;
    val = entryDefault;
    getConfig().Read(groupId + "/" + entryId, &val);
...
{
    getConfig().Write(groupId + "/" + entryId, m_pText->GetValue());
    return TRUE;
}
...
{
    getConfig().Write(groupId + "/" + entryId, m_pCombo->GetValue());
    return TRUE;
}
