...
#ifdef __GNUG__
#pragma implementation "dirctrlg_fix.h"
#endif
...
IMPLEMENT_DYNAMIC_CLASS(wxFixedDirCtrl, wxControl)

BEGIN_EVENT_TABLE(wxFixedDirCtrl, wxControl)
  EVT_TREE_ITEM_EXPANDING     (-1, wxFixedDirCtrl::OnExpandItem)
  EVT_TREE_ITEM_COLLAPSED     (-1, wxFixedDirCtrl::OnCollapseItem)
  EVT_TREE_BEGIN_LABEL_EDIT   (-1, wxFixedDirCtrl::OnBeginEditItem)
  EVT_TREE_END_LABEL_EDIT     (-1, wxFixedDirCtrl::OnEndEditItem)
  EVT_SIZE                    (wxFixedDirCtrl::OnSize)
END_EVENT_TABLE()

wxFixedDirCtrl::wxFixedDirCtrl(void)
{
    Init();
}

bool wxFixedDirCtrl::Create(wxWindow *parent,
                              const wxWindowID id,
                              const wxString& dir,
                              const wxPoint& pos,
                              const wxSize& size,
                              ...
                              const wxString& name)
{
    ...
    if (!filter.IsEmpty() && (style & wxDIRCTRL_SHOW_FILTERS))
        m_filterListCtrl = new wxFixedDirFilterListCtrl(this, wxID_FILTERLISTCTRL, wxDefaultPosition, wxDefaultSize, filterStyle);
...
wxFixedDirCtrl::~wxFixedDirCtrl()
{
}

void wxFixedDirCtrl::Init()
{
    m_showHidden = FALSE;
    m_imageList = NULL;
    m_currentFilter = 0;
...
void wxFixedDirCtrl::ShowHidden( bool show )
{
    m_showHidden = show;
...
void wxFixedDirCtrl::AddSection(const wxString& path, const wxString& name, int imageId)
{
...
void wxFixedDirCtrl::SetupSections()
{
...
#else
    #error "Unsupported platform in wxFixedDirCtrl!"
#endif
}

void wxFixedDirCtrl::OnBeginEditItem(wxTreeEvent &event)
{
...
void wxFixedDirCtrl::OnEndEditItem(wxTreeEvent &event)
{
...
void wxFixedDirCtrl::OnExpandItem(wxTreeEvent &event)
{
...
void wxFixedDirCtrl::OnCollapseItem(wxTreeEvent &event )
{
    CollapseDir(event.GetItem());
}

void wxFixedDirCtrl::CollapseDir(wxTreeItemId parentId)
{
    wxTreeItemId child;
...
void wxFixedDirCtrl::ExpandDir(wxTreeItemId parentId)
{
    wxDirItemData *data = (wxDirItemData *) m_treeCtrl->GetItemData(parentId);
...
void wxFixedDirCtrl::ReCreateTree()
{
...
wxTreeItemId wxFixedDirCtrl::FindChild(wxTreeItemId parentId, const wxString& path, bool& done)
{
    wxString path2(path);
...
bool wxFixedDirCtrl::ExpandPath(const wxString& path)
{
    bool done = FALSE;
...
wxString wxFixedDirCtrl::GetPath() const
{
...
wxString wxFixedDirCtrl::GetFilePath() const
{
    wxTreeItemId id = m_treeCtrl->GetSelection();
    if (id)
    {
...
void wxFixedDirCtrl::SetPath(const wxString& path)
{
    m_defaultPath = path;
    if (m_rootId)
        ExpandPath(path);
...
// Not used
#if 0
void wxFixedDirCtrl::FindChildFiles(wxTreeItemId id, int dirFlags, wxArrayString& filenames)
{
    wxDirItemData *data = (wxDirItemData *) m_treeCtrl->GetItemData(id);
...
void wxFixedDirCtrl::SetFilterIndex(int n)
{
    m_currentFilter = n;
...
void wxFixedDirCtrl::SetFilter(const wxString& filter)
{
    m_filter = filter;
...
bool wxFixedDirCtrl::ExtractWildcard(const wxString& filterStr, int n, wxString& filter, wxString& description)
{
...
int wxFixedDirCtrl::ParseFilter(const wxString& filterStr, wxArrayString& filters, wxArrayString& descriptions)
{
...
void wxFixedDirCtrl::OnSize(wxSizeEvent& WXUNUSED(event))
{
    DoResize();
}

//-----------------------------------------------------------------------------
// wxFixedDirFilterListCtrl
//-----------------------------------------------------------------------------

IMPLEMENT_CLASS(wxFixedDirFilterListCtrl, wxChoice)

BEGIN_EVENT_TABLE(wxFixedDirFilterListCtrl, wxChoice)
    EVT_CHOICE(-1, wxFixedDirFilterListCtrl::OnSelFilter)
END_EVENT_TABLE()

bool wxFixedDirFilterListCtrl::Create(wxFixedDirCtrl* parent, const wxWindowID id,
              const wxPoint& pos,
              const wxSize& size,
              long style)
{
...
void wxFixedDirFilterListCtrl::Init()
{
    m_dirCtrl = NULL;
}

void wxFixedDirFilterListCtrl::OnSelFilter(wxCommandEvent& WXUNUSED(event))
{
...
void wxFixedDirFilterListCtrl::FillFilterList(const wxString& filter, int defaultFilter)
{
    Clear();
